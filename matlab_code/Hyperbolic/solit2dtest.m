% Euclidean code for Jeremy's nonlinearity (e-mail)


function sol = solit2dtest(mu,R,N,L,sigma)
...
% for nonlin1 and dnonlin1:
     ...
     for j = 1:100
        ...
        lambda = lambda0 + ((sum(sum((absXIsquared + mu).^(-1).*fftshift(fftn(dnonlin1(ifftn(ifftshift(what0)),lambda0,sigma))).*conj(what0)))).^(-1))* ...
            (sum(sum((abs(what0).^2))) - sum(sum((fftshift(fftn(ifftn(ifftshift(what0)).*V))).*(absXIsquared+mu).^(-1).*conj(what0))) - ...
            sum(sum((absXIsquared + mu).^(-1).*fftshift(fftn(nonlin1(ifftn(ifftshift(what0)),lambda0,sigma))).*conj(what0))));
        
     
        q = abs(lambda - lambda0)
        lambda0 = lambda;
...
% (10) from Albowitz
    what = (fftshift(fftn((ifftn(ifftshift(what0))).*V)))./(absXIsquared+mu) + (fftshift(fftn(nonlin1(ifftn(ifftshift(what0)),lambda0,sigma))))./(absXIsquared+mu);
...
f = ((abs(lambda*z).^2)./(1+sigma*abs(lambda*z).^2)).*z;
...
f = ((((1+sigma*abs(lambda*z).^2).*(2.*lambda.*(abs(z).^2)))-((abs(lambda*z).^2).*(2.*sigma.*lambda.*(abs(z).^2))))./((1+sigma*abs(lambda*z).^2).^2)).*z;
