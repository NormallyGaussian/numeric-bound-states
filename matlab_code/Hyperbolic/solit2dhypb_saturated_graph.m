...
% For saturated nonlinearity

function sol = solit2dhypb_saturated_graph(mu,R,N,L,sigma)
...
err = 1;
    ...
    while (err > .000000001)
        ...
        lambda = lambda0 + ((sum(sum((absXIsquared + mu).^(-1).*fftshift(fftn(dnonlin1(ifftn(ifftshift(what0)),lambda0,X,Y,sigma))).*conj(what0)))).^(-1))* ...
            (sum(sum((abs(what0).^2))) - sum(sum((fftshift(fftn(ifftn(ifftshift(what0)).*V))).*(absXIsquared+mu).^(-1).*conj(what0))) - ...
            sum(sum((absXIsquared + mu).^(-1).*fftshift(fftn(nonlin1(ifftn(ifftshift(what0)),lambda0,X,Y,sigma))).*conj(what0))));
...
% (10) from Albowitz
    what = (fftshift(fftn((ifftn(ifftshift(what0))).*V)))./(absXIsquared+mu) + (fftshift(fftn(nonlin1(ifftn(ifftshift(what0)),lambda0,X,Y,sigma))))./(absXIsquared+mu);
...
f = (r./(sinh(r))).*((((1+sigma*abs(lambda*z).^2).*(2.*lambda.*(abs(z).^2)))-((abs(lambda*z).^2).*(2.*sigma.*lambda.*(abs(z).^2))))./((1+sigma*abs(lambda*z).^2).^2)).*z;
